...
```{r}
dlim_pred <- predict(dlim_fit, newdata = 0.5, type="CE")
...
```{r}
dlim_pred <- predict(dlim_fit, newdata = 0.5, type="DLF")
...
```{r}
plot_cumulative(new_modifiers = seq(0.1,0.9,0.1), mod_fit = dlim_fit, mod_name = "modifier")
```
...
```{r}
plot_DLF(new_modifiers = seq(0.1,0.9,0.1), mod_fit = dlim_fit, mod_name = "modifier", plot_by = "time", time_pts = c(10,20,30))
```
...
```{r}
plot_DLF(new_modifiers = c(0.25, 0.5, 0.75), mod_fit = dlim_fit, mod_name = "modifier", plot_by = "modifier")
```
